...
const Grainient = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      ...
      alpha: true,
      antialias: false,
      // A subtle fixed background does not benefit from retina-resolution
      // rendering. This cuts its pixel workload substantially on 2x displays.
    ...
    const loop = t => {
      if (!ctx.animated || !isVisible || !isPageVisible) {
        raf = 0;
        return;
      }
    ...
    const tryStart = () => {
      if (ctx.animated && isVisible && isPageVisible && raf === 0) raf = requestAnimationFrame(loop);
    };
    const tryStop = () => {
  ...
  useEffect(() => {
    ...
    // The configured site background is intentionally static. Render once
    // after prop updates and only run a frame loop when time/grain actually
    // changes, instead of spending GPU time on identical frames forever.
    ctx.animated = timeSpeed !== 0 || grainAnimated;
    renderer.render({ scene: mesh });
    if (ctx.animated) ctx.start();
    else ctx.stop();
  }, [
